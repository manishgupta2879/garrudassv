	...
	if (isset($array['HEADER']['TALLYREQUEST'])) {
		...
		switch ($actions) {
			...
			case "ALL VOUCHERS":
				...
				$sql = "SELECT rid FROM pre_distributors WHERE id ='$LGBILLINGCODE' and password='$LGOTP' and pid='$PID'";
				...
				if ($distributor_varification) {
					...
					if ($array['BODY']['DATA']['REQUESTDATA']['TALLYMESSAGE']) {
						...
						foreach ($vals as $val) {
							...
							$val['CREATIONDATE'] = $val['CREATIONDATE'] ? date("Y-m-d", strtotime($val['CREATIONDATE'])) : date("Y-m-d", strtotime($val['DOCDATE']));
							$val['ALTEREDON'] = $val['ALTEREDON'] ? date("Y-m-d", strtotime($val['ALTEREDON'])) : "0000-00-00";
							$val['DOCDATE'] = $val['DOCDATE'] ?	date("Y-m-d", strtotime($val['DOCDATE'])) : "0000-00-00";
							$val['DFIELD1'] = $val['DFIELD1'] ?	date("Y-m-d", strtotime($val['DFIELD1'])) : "0000-00-00";
							$val['DFIELD2'] = $val['DFIELD2'] ?	date("Y-m-d", strtotime($val['DFIELD2'])) : "0000-00-00";
							$val['DFIELD3'] = $val['DFIELD3'] ?	date("Y-m-d", strtotime($val['DFIELD3'])) : "0000-00-00";
							$val['DFIELD4'] = $val['DFIELD4'] ?	date("Y-m-d", strtotime($val['DFIELD4'])) : "0000-00-00";
							$val['DFIELD5'] = $val['DFIELD5'] ?	date("Y-m-d", strtotime($val['DFIELD5'])) : "0000-00-00";
							...
							if (!in_array($val['TRNTYPE'], array('Sales Removed', 'Credit Note Removed', 'Purchase Removed', 'Debit Note Removed', 'Stock Journal Removed'))) {
								...
								foreach ($rows as $row) {
									...
									if ($deletedQty) {
										$q = "select id from pre_stockreports where pid = '$PID' AND  distid = '$LGBILLINGCODE' AND itemcode='$rowStockNo' AND YEAR(`fromdate`)='$rowYear' AND MONTH(`fromdate`)='$rowMonth' and itemgodownname='' and itembatchname='' ORDER BY id ASC LIMIT 1";
									...
									$empty_godownname_array_ids[] = $rowID;
							...
							foreach ($inventy as $data) {
								...
								$q = "select id,product_name from pre_product_management where '$itemname' IN (`id`, `sfield6`, `sfield7`, `sfield8`, `sfield9`)";
								$products = selectone($q);
								if ($products) {
									$code = $products['id'];
									# check entry in stock table start
									$q = "select id from pre_stockreports where distid = '$LGBILLINGCODE' AND itemcode='$code' AND YEAR(`fromdate`)='$year' AND MONTH(`fromdate`)='$month' and itemgodownname='' and itembatchname='' ORDER BY id ASC LIMIT 1";
									$stock = selectone($q);
									...
									if (strtolower($ttype) == 'sales') {
										...
										if ($selectdistsync1) {
											$ds_id = $selectdistsync1['id'];
											$sql = "UPDATE pre_distributor_syncs set sale_date = '$sale_date' WHERE id = '$ds_id'";
											mysqli_query($link, $sql);
										} else {
											//BATCH QUERY
											//$pre_distributor_syncs_sale_date .= "('$d_code','$sale_date'), ";
											$sql = "INSERT INTO pre_distributor_syncs(distributor_id,sale_date) values ('$d_code','$sale_date')";
											mysqli_query($link, $sql);
										}
									}
									...
									$trnType = $val['TRNTYPE'];
									$masterId = $val['MASTERID'];
									$q = "SELECT count(*) FROM pre_transaction_errors WHERE pid = '$PID'  AND disti_code = '$distCode' AND masterid = '$masterId' AND trntype = '$trnType' AND STOCKNO = '$code'";
									$hasTransaction = countRecords($q);
									if (!$hasTransaction) {
										$val['ENTSLRNO'] = $data['ENTSLRNO'];
										$val['STOCKNO'] = $code;
						...
						// UPDATE Transactions QUANTITY IF ANY
						// $empty_godownname_array_ids[] = $rowID;
						if (count($empty_godownname_array_ids)) {
							$empty_godownname_array_ids = array_unique($empty_godownname_array_ids);
							$ids = implode(',', $empty_godownname_array_ids);
							$sqlInsert = "UPDATE pre_transactions SET stkqty='0',stkaltqty='0',`bw_posted`='0',stkrate='0',stkvalue='0',amount='0' ,is_deleted = '1' where id IN ('$ids') AND godownname = ''";
							$res = mysqli_query($link, $sqlInsert);

							$sqlInsert = "UPDATE pre_transactions SET batchslrno='0',batchmfgdate='0000-00-00', batchexpirydate='0000-00-00',batchqty='0',batchaltqty='0',alternatebatchqty='0',alternatebatchaltqty='0',
								tailbatchqty='0',tailbatchaltqty='0',batchrate='0', is_deleted = '1',batchvalue='0',`bw_posted`='0',amount='0' where id IN ('$ids') AND godownname != '' ";
							$res = mysqli_query($link, $sqlInsert);
						}

						if (count($update_closing_stock)) {
							updateClosingStockIds($link, $update_closing_stock);
						}


						$curdate = date("Y-m-d");
						$selectdistsync = countRecords("SELECT count(*) FROM pre_distributor_syncs WHERE distributor_id = '$d_code' and sync_date = '$curdate'");
						if (!$selectdistsync) {
					...
					foreach ($DATA as $pKey => $productData) {
						...
						if ($products) {
							...
							if (isset($productData['PRODUCTCODE'])) {
								...
								$array['BODY']['DATA']['REQUESTDATA']['STATUS'] = 'Product Code Not Found';
								$array['BODY']['DATA']['REQUESTDATA']['PRODUCTCODE'] = $product_code;
								$array['BODY']['DATA']['REQUESTDATA']['PRODUCTPRIMARYCODE'] = '';
							}
						}
					}
...
function updateClosingStockIds($link, $ids)
{
